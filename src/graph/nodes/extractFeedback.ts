...
export const extractFeedback = async (state: StateType, config?: RunnableConfig) => {
  logger.info("----------------------------------------------------------------");
  ...
  const transcriptText = state.transcript.map((t: any, i: number) => `[${i}] ${t.role}: ${t.text}`).join("\n");
...
${transcriptText}`;
  ...
  logger.info(
    `  [1/4] Phase 1 Complete. Extracted: ${validated.issues.length} issues, ${validated.positives.length} positive remarks.`
  );
  return {
