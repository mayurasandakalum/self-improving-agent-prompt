...
function getMockResponse(opts) {
        ...
        if (userMsg.includes("cancel my order") && userMsg.includes("don't know how to do that")) {
            ...
            return JSON.stringify({
                issues: [
                    {
                        type: "tool_failure",
                        description: "The agent explicitly stated they do not know how to cancel the order, despite the customer's request.",
        ...
        if (userMsg.includes("hawaii") || userMsg.includes("maui")) {
            return JSON.stringify({
                issues: [
                    {
                        type: "off_script",
                        description: "Agent engaged in lengthy chit-chat about vacation and failed to steer the conversation back to the main booking task.",
    ...
    if (systemMsg.includes("root causes in the system prompt") || systemMsg.includes("shouldupdate")) {
                ...
                rootCauses: [
                    {
                        ...
                        cause: "The system prompt does not instruct the agent on how to handle order cancellations or use the order cancellation tool.",
                        affectedPromptSection: "missing",
                        suggestedChange: "add",
                    },
                ],
                shouldUpdate: true,
                reasoning: "Prompt needs to be updated with explicit instructions to handle order cancellations and use appropriate order cancellation tools.",
            });
        }
                ...
                rootCauses: [
                    {
                        ...
                        cause: "Prompt asks the agent to confirm the name but does not instruct them to write down or memorize it upon first mention.",
        ...
        return JSON.stringify({
            rootCauses: [],
            shouldUpdate: false,
            reasoning: "The conversation was completed successfully with no issues identified. Prompt is functioning optimally.",
        });
    }
    ...
    if (systemMsg.includes("rewrite the system prompt") || systemMsg.includes("changelog")) {
        ...
        const originalPromptMatch = opts.userMessage.match(/(?:Current System Prompt|Current Prompt):\s*\n([\s\S]*?)\n\n(?:Engineering Analysis of Issues|Analysis):/i);
        ...
        if (userMsg.includes("cancel") || userMsg.includes("tool_failure")) {
            ...
            newPrompt = originalPrompt.replace("Verify user details before proceeding.", "Verify user details once. Do not repeat questions if customer already gave information.");
            ...
            newPrompt = originalPrompt.replace("Push the benefits of our premium plan.", "Answer pricing inquiries directly if asked, then highlight the benefits of our premium plan.");
            ...
            newPrompt = originalPrompt.replace("Be friendly and conversational.", "Be friendly and conversational, but keep chit-chat minimal. Pivot back to the booking flow within one turn if user goes off-topic.");
