...
export const validatePrompt = (state, config) => {
    ...
    if (proposed.length < minLength) {
        failures.push(`Proposed prompt is too short (${proposed.length} chars) compared to original (${original.length} chars). Must be at least 50% (${Math.round(minLength)} chars).`);
    }
    if (proposed.length > maxLength) {
        failures.push(`Proposed prompt is too long (${proposed.length} chars) compared to original (${original.length} chars). Must be at most ${Math.round(maxLength)} chars.`);
    }
    ...
    if (originalLineCount >= 5 && changeRatio > maxAllowedRatio) {
        failures.push(`Too many prompt modifications. Changed lines: ${changedLinesCount} out of ${originalLineCount} total original lines (${Math.round(changeRatio * 100)}%). Max allowed is 40%.`);
    }
    else if (originalLineCount < 5 && changedLinesCount > 15) {
        failures.push(`Too many prompt modifications for a short prompt. Changed lines: ${changedLinesCount}. Max allowed is 15 lines.`);
    }
    ...
    if (lowerProposed.includes("...truncated") || lowerProposed.includes("[continued]")) {
        failures.push("Proposed prompt appears corrupted or truncated (contains placeholder text like '...truncated' or '[continued]').");
    }
